...
def test_get_rules_success(db_session):
    ...
    regulation2 = Regulation(
            title="테스트 규정 Beta", type="규정", create_date=datetime.datetime(2023, 2, 1), # Changed "지침" to "규정"
    ...
    # TestClient by default uses "http://testserver" as the base URL for requests.
    expected_web_url = f"http://testserver/regulation/{regulation1.html_url}"
    ...
    # Find the card with "테스트 규정 Beta" title
    card_beta = None
    for item in response_beta_json["template"]["outputs"][0]["carousel"]["items"]:
        if item["title"] == "테스트 규정 Beta":
            card_beta = item
            break
    assert card_beta is not None, "Card with title '테스트 규정 Beta' not found in response"
